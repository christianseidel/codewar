...
public class SumOfIntervalsOld {
/*
 ...
 */
    ...
    private static int[][] eliminateOneLevelOfOverlap(int[][] ledger) {
        for (int m = 0; (m + 1) < ledger.length; m++) {
            ...
            // check if there is overlap on one side
            if (difference < 0) {
                // check if there is overlap on both sides
                if (ledger[m][1] > ledger[m+1][1]) {
                    // if there is overlap on both sides, m+1 will be disregarded
                    ledger[m+1][2] = ledger[m+1][3] = 0;
                    break;
                } else {
                    // if there is overlap on one side only, m+1 gets shortened
    ...
    private static int getSumOfIntervalsToEliminate(int[][] ledger) {
        int sum = 0;
    ...
    private static int[][] shrinkLedger(int[][] ledger) {
